...
def get_chat_response(profile: dict, metrics: dict, user_message: str, history: list, metrics_summary: dict = None) -> str:
    tx_context = ""
    if metrics_summary and metrics_summary.get('transaction_count', 0) > 0:
        top_cats = metrics_summary.get('top_spending_categories', {})
        top_cats_str = ', '.join([f"{k}: ₹{v:,.0f}" for k, v in top_cats.items()]) if top_cats else 'None recorded'
        tx_context = f"""
...
def get_simulator_explanation(profile: dict, item_name: str, simulation_result: dict) -> str:
